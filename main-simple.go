...
func (s *SystemTuneAgent) Start() error {
	fmt.Printf("System Tune Agent 启动 (Go 简化版)\n")
...
func (s *SystemTuneAgent) getCPUUsage() (float64, error) {
	...
	for _, line := range lines {
		if strings.Contains(line, "CPU usage:") {
			// 解析类似 "CPU usage: 10.5% user, 2.3% sys, 87.2% idle" 的行
	...
	return 0, fmt.Errorf("无法解析 CPU 使用率")
}
...
func (s *SystemTuneAgent) getMemoryUsage() (float64, error) {
	...
	var pageSize, freePages, inactivePages int64 = 4096, 0, 0
	...
	for _, line := range lines {
		if strings.Contains(line, "page size of") {
			...
			parts := strings.Fields(line)
			if len(parts) >= 3 {
				freeStr := strings.TrimSuffix(parts[2], ".")
				if pages, err := strconv.ParseInt(freeStr, 10, 64); err == nil {
					freePages = pages
				}
			}
			...
			parts := strings.Fields(line)
			if len(parts) >= 3 {
				inactiveStr := strings.TrimSuffix(parts[2], ".")
				if pages, err := strconv.ParseInt(inactiveStr, 10, 64); err == nil {
					inactivePages = pages
				}
			}
		}
	...
	totalMemory, err := strconv.ParseInt(strings.TrimSpace(string(output)), 10, 64)
	if err != nil {
		return 0, err
	}
	...
	return float64(usedPages) / float64(totalPages) * 100, nil
}
...
func (s *SystemTuneAgent) startMemoryConsumption() {
	for s.shouldConsumeMemory {
		...
		currentUsage, err := s.getMemoryUsage()
		if err != nil {
...
func main() {
	...
	if *showHelp {
		fmt.Println("System Tune Agent - 系统资源调节工具 (Go 简化版)")
